...
class Model(pixeldp.Model):
    ...
    def _build_model(self, inputs_tensor=None, labels_tensor=None):
        ...
        with tf.variable_scope('predictions'):
            self.predictions = self._fully_connected(x, 1)
            self.predictions = self._relu(self.predictions)
        ...
        with tf.variable_scope('costs'):
            self.cost =tf.losses.mean_squared_error(predictions=self.predictions, labels=labels)
